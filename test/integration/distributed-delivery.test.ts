...
async function deliverNext(consumer: SqsConsumer, queues: Awaited<ReturnType<typeof createTestQueues>>) {
...
async function eventuallyReceive(
  ...
  for (const _ of [undefined, undefined, undefined]) {
    const message = await receiveOne(queues.client, queues.wagers);
...
integration('handles persistent redelivery, outbox leases, DLQ, pending references and worker restart', async () => {
  ...
  try {
    await withDisposablePostgres(async (databaseUrl) => {
      ...
      const orm = await MikroORM.init({ clientUrl: databaseUrl, entities: [SchemaMigrationEntity] });
      ...
      try {
        ...
        expect(JSON.parse(openingEvent?.Body ?? '{}')).toMatchObject({ eventType: expect.any(String), version: 1 });
        ...
        await sendCommand(queues.wagers, transient, 'transport-transient-bet', wallet.id, queues.client);
        await deliverNext(consumer, queues);
        ...
        await sendCommand(queues.wagers, refund, 'transport-refund-before-bet', wallet.id, queues.client);
        await deliverNext(consumer, queues);
        await sendCommand(
          queues.wagers,
          wagerEnvelope({
