...
CREATE TABLE wallets (
 ...
 CONSTRAINT wallets_player_currency_uk UNIQUE (player_id,currency), CONSTRAINT wallets_identity_uk UNIQUE (id,player_id,currency),
...
CREATE TABLE wager_transactions (
 ...
 CONSTRAINT wager_idempotency_uk UNIQUE (idempotency_key), CONSTRAINT wager_provider_external_uk UNIQUE (provider_id,external_transaction_id),
...
CREATE TABLE wallet_ledger_entries (
 id uuid PRIMARY KEY, wallet_id uuid NOT NULL REFERENCES wallets(id), transaction_id uuid NOT NULL REFERENCES wager_transactions(id), direction varchar(6) NOT NULL,
...
`,
  down: `DROP TABLE IF EXISTS outbox_messages; DROP TABLE IF EXISTS inbox_messages; DROP TABLE IF EXISTS wallet_ledger_entries; DROP TABLE IF EXISTS wager_transactions; DROP TABLE IF EXISTS wallets; DROP TABLE IF EXISTS schema_migrations;`,
};
